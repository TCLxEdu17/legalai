    ...
    <div className="flex flex-col h-full bg-[#0d0d0d]">
      ...
      <div className="border-b border-white/[0.06] px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-slate-100 font-semibold text-base">Meus Casos</h1>
            <p className="text-slate-500 text-xs">{cases.length} caso{cases.length !== 1 ? 's' : ''} cadastrado{cases.length !== 1 ? 's' : ''}</p>
          </div>
        </div>
        <button
          onClick={() => setShowModal(true)}
          className="flex items-center gap-2 px-3 py-2 bg-brand-600 hover:bg-brand-500 text-white text-sm rounded-lg transition-colors"
        >
      ...
      <div className="px-6 py-3 border-b border-white/[0.04]">
        <div className="relative">
          ...
          <input
            ...
            className="w-full bg-white/[0.03] border border-white/[0.08] rounded-lg pl-9 pr-4 py-2 text-sm text-slate-300 placeholder-slate-600 focus:outline-none focus:border-brand-500/50"
          />
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center h-60 text-center">
            ...
            <p className="text-slate-400 font-medium">Nenhum caso encontrado</p>
            <p className="text-slate-600 text-sm mt-1">
              {search ? 'Tente outros termos de busca.' : 'Crie seu primeiro caso jurídico para começar.'}
            </p>
            {!search && (
              <button
                onClick={() => setShowModal(true)}
                className="mt-4 flex items-center gap-2 px-4 py-2 bg-brand-600/20 hover:bg-brand-600/30 border border-brand-500/30 text-brand-400 text-sm rounded-lg transition-colors"
              >
                <Plus className="w-4 h-4" />
                Criar primeiro caso
              </button>
          ...
          <div className="grid grid-cols-1 gap-3">
            {filtered.map((c) => (
              <div
                key={c.id}
                className="group relative bg-white/[0.02] border border-white/[0.06] rounded-xl p-4 hover:border-white/[0.12] hover:bg-white/[0.04] transition-all cursor-pointer"
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <h3 className="text-slate-200 font-medium text-sm truncate">{c.title}</h3>
                    ...
                    {c.processNumber && (
                      <p className="text-slate-600 text-xs mt-0.5">Proc: {c.processNumber}</p>
                    )}
                  </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-[#141414] border border-white/[0.08] rounded-2xl w-full max-w-lg">
            ...
            <div className="p-5 space-y-4">
              <div>
                <label className="text-slate-400 text-xs font-medium block mb-1">Título do caso *</label>
                <input
                  value={form.title}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-slate-400 text-xs font-medium block mb-1">Área jurídica</label>
                  <select
                    value={form.area}
                ...
                <div>
                  <label className="text-slate-400 text-xs font-medium block mb-1">Nº do processo</label>
                  <input
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-slate-400 text-xs font-medium block mb-1">Autor / Requerente</label>
                  <input
                ...
                <div>
                  <label className="text-slate-400 text-xs font-medium block mb-1">Réu / Requerido</label>
                  <input
              ...
              <div>
                <label className="text-slate-400 text-xs font-medium block mb-1">Tribunal / Vara</label>
                <input
                  value={form.court}
              ...
              <div>
                <label className="text-slate-400 text-xs font-medium block mb-1">Notas iniciais</label>
                <textarea
                  value={form.notes}
