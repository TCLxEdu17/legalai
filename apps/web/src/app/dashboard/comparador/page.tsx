...
import { GitCompare, Search, X, Loader2, Sparkles, FileText } from 'lucide-react';
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-slate-500 text-sm mt-1">Selecione 2 a 4 jurisprudências para comparar com IA</p>
      </div>
        ...
        {selected.length > 0 && (
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              {selected.map((doc, i) => (
                <div key={doc.id} className="flex items-start gap-2 p-2.5 bg-brand-600/5 border border-brand-500/20 rounded-lg">
                  <span className="text-[10px] bg-brand-600/20 text-brand-400 rounded px-1.5 py-0.5 shrink-0 font-bold">{i + 1}</span>
      ...
      {selected.length < 2 && (
        <div className="flex flex-col items-center justify-center py-12 text-center bg-[#141414] border border-white/[0.07] rounded-xl">
          ...
          <p className="text-slate-400 font-medium">Selecione ao menos 2 jurisprudências</p>
          <p className="text-slate-500 text-sm mt-1">Use a busca acima para encontrar documentos</p>
        </div>
      )}
    </div>
  );
