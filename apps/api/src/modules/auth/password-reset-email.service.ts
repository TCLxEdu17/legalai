...
export class PasswordResetEmailService {
  ...
  async sendWelcomeEmail(email: string, name: string): Promise<void> {
    if (!this.resend) {
      this.logger.warn(`Welcome email não enviado para ${email} — Resend não configurado`);
      return;
    }
    ...
    try {
      await this.resend.emails.send({
        from: this.fromEmail,
        to: email,
        subject: 'Bem-vindo ao LegalAI 🎉',
        html: buildWelcomeEmailHtml({ name, loginUrl: `${this.frontendUrl}/login` }),
      });
      this.logger.log(`Welcome email enviado para ${email}`);
    } catch (err: any) {
      this.logger.error(`Falha ao enviar welcome email: ${err?.message}`);
    }
  }
...
function buildWelcomeEmailHtml(data: { name: string; loginUrl: string }): string {
  return `<!DOCTYPE html>
<html lang="pt-BR">
      ...
      <table width="560" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 1px 3px rgba(0,0,0,.08)">
        <tr>
          <td style="background:linear-gradient(135deg,#1e3a8a,#2563eb);padding:28px 32px">
            <p style="margin:0;color:#bfdbfe;font-size:12px;font-weight:600;letter-spacing:1px;text-transform:uppercase">LegalAI · Bem-vindo</p>
            <h1 style="margin:6px 0 0;color:#fff;font-size:22px;font-weight:700">Sua conta foi criada!</h1>
          </td>
        </tr>
        <tr>
          <td style="padding:32px">
            ...
            <p style="margin:0 0 24px;color:#64748b;font-size:14px;line-height:1.6">
              Sua conta no LegalAI foi criada com sucesso. Agora você tem acesso ao assistente jurídico com IA
              mais completo para escritórios de advocacia brasileiros.
            </p>
